'''
Example taken from Balaji et al. and modified as part of ORSUITE: https://github.com/cornell-orie/ORSuite
...
class BinPackingEnv(gym.Env):
    ...
    def _STEP(self, action):
        ...
        return self.state, reward, done, {'item_size': self.item_size}
...
class BinPackingToy(BinPackingEnv):
    '''
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.bin_capacity = 6 # from 4
        # self.item_probs = [.25, .3, .3, .15]
        self.item_probs = [.1, .1, .4, .4]
        self.item_sizes = np.arange(1, 5)
        self.step_limit = 10
        self._build_obs_space()
        self._check_settings()
        self.seed()
